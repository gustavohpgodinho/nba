...
tokenizer_plays <- function(df){
  ...
  obj7 <- obj %>% 
    dplyr::left_join(obj6 %>% 
                       dplyr::select(joincod, row, si), 
                     by = c('joincod', 'row')) %>% 
    dplyr::select(-c(n, joincod, row, importance)) %>% 
    dplyr::select(season, game_id, period, clock, si, cods, clean_cod, codf) %>% 
    dplyr::distinct() %>% 
    dplyr::arrange(season, game_id, period, desc(clock), si) %>% 
    dplyr::group_by(season, game_id, period) %>% 
    dplyr::mutate(lag_codf = dplyr::lag(codf)) %>% 
    dplyr::ungroup()
}
